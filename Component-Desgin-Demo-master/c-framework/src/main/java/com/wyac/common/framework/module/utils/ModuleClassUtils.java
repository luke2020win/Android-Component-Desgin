package com.wyac.common.framework.module.utils;
    ...
    public static <T> List<T> getObjectsWithClassName(Context context, Class<T> clazz, List<String> classNameList) {
        ...
        try {
            for (String className : classNameList) {
                Class aClass = Class.forName(className);
                if (clazz.isAssignableFrom(aClass) && !clazz.equals(aClass) && !aClass.isInterface()) {
                    EFLog.e(TAG, "getObjectsWithClassName add className:"+className);
                    objectList.add((T) Class.forName(className).getConstructor().newInstance());
                }
            }

            if (objectList.size() == 0) {
                EFLog.e(TAG, "getObjectsWithClassName No files were found, check your configuration please!");
            }
        } catch (Exception e) {
            e.getStackTrace();
            EFLog.e(TAG, "getObjectsWithClassName error, " + e.getMessage());
        }
