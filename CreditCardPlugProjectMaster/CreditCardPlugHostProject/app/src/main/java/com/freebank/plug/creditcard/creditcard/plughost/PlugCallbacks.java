...
public class PlugCallbacks extends RePluginCallbacks {
    private static final String TAG = "PlugCallbacks";

    private ClassLoader mOriginClassLoader;

    public PlugCallbacks(Context context) {
        super(context);
    }
    ...
    public PluginDexClassLoader createPluginClassLoader(PluginInfo pi, String dexPath, String optimizedDirectory, String librarySearchPath, ClassLoader parent) {
        DLog.i(TAG, "createPluginClassLoader() PluginExtra: " + pi + ", dexPath: " + dexPath + ", optimizedDirectory: " + optimizedDirectory + ", librarySearchPath: " + librarySearchPath + ", parent: " + parent.getClass().getName());
        ...
        /** hook classLoader的native加载目录（可以增加判断条件，仅针对so插件做）*/
    ...
    public boolean onPluginNotExistsForActivity(Context context, String plugin, Intent intent, int process) {
        DLog.i(TAG, "onPluginNotExistsForActivity() plugin: " + plugin + ", process: " + process+ ", context: " + context + ", intent: " + intent);
